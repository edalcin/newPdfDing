...
Dois botões na página de detalhes (`/pdf/[id]`), habilitados apenas quando `GEMINI_API_KEY` está configurada **e** um modelo de texto foi escolhido em Configurações → IA (senão a mensagem `412` do backend aparece no lugar de falhar em silêncio — ver [API](05-api.md#modelos-de-ia)):

- **"Descrever com IA"** (ao lado do rótulo Descrição): `POST /api/pdfs/{id}/describe` usando o texto já extraído do PDF; a resposta preenche o campo e é salva imediatamente pelo mesmo `PATCH /api/pdfs/{id}` que o blur do campo já usa — sem caminho de escrita separado. Pede confirmação antes de substituir uma descrição não vazia.
- **"Sugerir tags"** (ao lado do rótulo Tags): `POST /api/pdfs/{id}/suggest-tags`; as sugestões (apenas tags já existentes, nunca inventadas — filtro no servidor) aparecem como chips clicáveis abaixo do `TagPicker`; clicar um chip aplica a tag via `saveTags` (mesmo caminho do combobox) e remove o chip da lista.

A seleção de modelo (embedding e texto) fica em **Configurações → IA** (`/settings`): dois `<select>` populados por `GET /api/ai/models`, que lista o catálogo real da chave configurada. O valor salvo aparece mesmo que `models.list` não o devolva mais (chave trocada, modelo retirado do catálogo) — a lista recebe uma entrada sintética `"<modelo> (indisponível)"` nesse caso. Sem `GEMINI_API_KEY`, a seção mostra um aviso e os dois seletores ficam desabilitados.
