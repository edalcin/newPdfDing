...
Este é o contrato de "nenhuma funcionalidade perdida" da refatoração. Cada linha é um item do levantamento feito sobre o código Django hoje em produção — enumeração canônica de rotas em `pdfding/pdf/urls.py`, `pdfding/admin/urls.py`, `pdfding/core/urls.py` e `pdfding/users/urls.py` — apontando para onde a mesma capacidade passa a viver na arquitetura Go + SvelteKit e em qual etapa ela foi entregue. Nenhuma funcionalidade listada abaixo é descartada sem estar também na seção final de remoções intencionais.

A coluna "Etapa" nomeia a etapa do plano de execução em que o item foi entregue; o documento desse plano (`ETAPAS.md`) foi apagado após a conclusão da refatoração — histórico disponível no git.
...
|---|---|---|---|
| Upload individual | `pdfding/pdf/views/pdf_views.py:Add` | `POST /api/pdfs` (multipart `file`, `preview`, `text`, `name`, `description`, `tags`) — [API](05-api.md); pré-processamento (preview/texto) roda no pdf.js do navegador antes do envio — [Frontend](06-frontend.md) | ETAPA-4-DOMINIO-PDF |
...
| Modo árvore de tags com `/` | `pdfding/users/models.py:Profile.tag_tree_mode` | **Nunca implementado.** Planejado como `settings['ui.tag_tree_mode']` — [Modelo de Dados](02-modelo-de-dados.md); não existe chave, toggle ou tela em árvore no produto. A hierarquia por `/` no nome da tag é preservada na normalização (`ParseTagString`), mas não há visualização em árvore. | ETAPA-10-UI-COMPLETA |
...
| Mover de coleção | `pdfding/pdf/services/collection_services.py:change_collection_of_pdf` | **Nunca implementado** — dependia de Coleções, que nunca foram implementadas. Ver linha "Coleções CRUD" abaixo. | ETAPA-4-DOMINIO-PDF |
...
| Funcionalidade | Onde está hoje (arquivo:símbolo) | Destino | Etapa |
|---|---|---|---|
| Thumbnail | `pdfding/pdf/views/pdf_views.py:ServeThumbnail` | **Nunca implementado.** Só existe Preview (linha abaixo); o PNG de preview gerado no upload também serve de miniatura na biblioteca — não há um asset de thumbnail separado nem rota `GET/POST /api/pdfs/{id}/thumbnail`. | ETAPA-4-DOMINIO-PDF |
| Preview | `pdfding/pdf/views/pdf_views.py:ServePreview`, `pdfding/pdf/views/pdf_views.py:ShowPreview` | `GET /api/pdfs/{id}/preview` — [API](05-api.md); chave `preview/{pdf_id}.png` — [Storage](03-storage.md) | ETAPA-4-DOMINIO-PDF |
...
| Assinaturas | `pdfding/users/models.py:Profile.signatures`, `pdfding/users/views.py:Signatures` | **Nunca implementado.** Planejada como tabela `signatures` — [Modelo de Dados](02-modelo-de-dados.md); não existe no schema atual nem rota `/api/signatures`. | ETAPA-5-ANOTACOES |
...
| Coleções CRUD | `pdfding/pdf/views/collection_views.py` | **Nunca implementado.** Planejada como `GET/POST/PATCH/DELETE /api/collections` e tabela `collections` — [Modelo de Dados](02-modelo-de-dados.md); nenhuma das duas existe no produto. | ETAPA-4-DOMINIO-PDF |
| Coleção padrão | `pdfding/pdf/models/collection_models.py:Collection.default_collection` | **Nunca implementado** — dependia de Coleções CRUD, acima. | ETAPA-4-DOMINIO-PDF |
...
| Informações da instância | `pdfding/admin/views.py:Information` | `GET /api/admin/info` (contagens de PDFs/tags, espaço sob `FILES`, contagem por `embedding_status`) — [API](05-api.md); tela `/admin` — [Frontend](06-frontend.md). Não devolve contagem de coleções — a funcionalidade de coleções nunca foi implementada. | ETAPA-10-UI-COMPLETA |
...
| Regeneração de thumbnails | `pdfding/pdf/management/commands/regenerate_thumbnails.py` | Substituído por `POST /api/admin/reindex` (reconstrói somente o índice FTS5) — [API](05-api.md), [Busca Híbrida](04-busca-hibrida.md). A função original do comando (gerar miniatura) não tem equivalente: thumbnail nunca foi implementado, ver linha "Thumbnail" acima — o PNG de preview gerado no upload já serve de miniatura. | ETAPA-8-BACKGROUND |
