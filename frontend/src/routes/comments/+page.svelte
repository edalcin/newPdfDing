...
<div class="mx-auto max-w-3xl p-4">
	<div class="flex items-center justify-between">
		<h1 class="text-lg font-semibold">Comentários</h1>
		<div class="flex gap-3 text-sm">
			<a class="text-muted-foreground hover:text-foreground" href={exportAnnotationsUrl('comment', '', 'json')} download>JSON</a>
			<a class="text-muted-foreground hover:text-foreground" href={exportAnnotationsUrl('comment', '', 'yaml')} download>YAML</a>
		</div>
	</div>

	{#if store.items.length === 0 && !store.loading}
		<p class="mt-8 text-center text-sm text-muted-foreground">Nenhum comentário ainda.</p>
	{:else}
