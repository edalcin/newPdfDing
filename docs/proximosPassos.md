...
- **Herdado do estudo 12, apagado nesta sessão.** O documento `refatoracao/12-achatamento-cartografia-vetorial.md` foi removido (plano cumprido; história no git), mas guardava dois achados de 2026-07-27 que continuam sem investigação e não devem sumir com ele:
  1. **`POST /api/auth/login` travou indefinidamente** quando exercitado por script contra produção — nem erro nem sucesso, enquanto as outras rotas respondiam em milissegundos. Não reproduzido depois; **não confirmado**, e desde então o login pelo navegador sempre funcionou, inclusive nas verificações de hoje. Suspeita registrada na época: pool SQLite de conexão única. Diagnosticar exigiria imagem de debug com dump de goroutines — inferência por eliminação não fecha.
  2. **O maior PDF do acervo** (`019f9ed9-b63b-7829-903e-85200ad43ff7`, 108 páginas, ~941 mil operações de path numa única página) continua **intocado**, com MD5 verificado idêntico ao original. A receita de achatamento (Ghostscript `-sDEVICE=jpeg -dJPEGQ=78 -r200` + `img2pdf`, em container descartável) foi testada e validada, e o reenvio usaria `PUT /api/pdfs/{id}/file`. Riscos que a receita exige checar antes: contagem de páginas idêntica e tamanho físico de página idêntico — anotações guardam retângulos em pontos e desalinham se a página mudar. Não existe UI para substituir arquivo; `PUT .../file` não é chamado por nenhum componente do frontend.
...
- **Funcionalidades que a especificação prometia e o código nunca teve** foram marcadas como "nunca implementado" no inventário e removidas dos contratos: **Coleções**, **thumbnail** (só existe preview), **assinaturas salvas** e **modo árvore de tags**. Não são pendências: são ficção documental corrigida. Reintroduzir qualquer uma é decisão nova, não dívida.
- **A pasta `refatoracao/` mantém o nome**, apesar de a refatoração estar concluída: renomear invalidaria ~120 citações a `refatoracao/NN-*.md` em comentários de código, churn sem ganho. Os documentos `00`–`08`, `10`, `11` são hoje a especificação do produto, não um plano.
- **Apagados nesta sessão, com aprovação:** `ETAPAS.md` e `09-limpeza-repositorio.md` (plano de execução cumprido), `12-achatamento-cartografia-vetorial.md` (estudo datado; achados pendentes resgatados na seção 5), `instrucoesFinais.md` (runbook de migração Django→Go, executado), e o importador legado inteiro (`import_legacy.go`, `-import-legacy`, `PDFStore.Import`, `AnnotationStore.Import`, `ShareStore.Import`). História no git.
